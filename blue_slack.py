        ...
        if event['subtype']!=None and message['channel'][0]!='C':
            ...
            if envia_bh==1:                
                ...
                for i in df.index:
                    ...
                    try:
                        ...
                        if canal_gestor!='None':
                            bancohoras=rdTimeBalance.post(str(df['Nome'][i]),str(df['E-mail Profissional'][i]),str(df['Nome do Gestor'][i]),str(df['E-mail Gestor'][i]),str(df['Banco Total'][i]).replace("=",""),str(canal_gestor).replace("[","").replace("]","").replace("'",""))
                    except:
                ...
                for i in df_grp.index:
                    ...
                    try:
                        result=client.files_upload(channels=str(df_grp['CHANNEL_ID'][i]),initial_comment=':neon: :hourglass_flowing_sand: BLUE - BANCO DE HORAS :hourglass: :neon: \n:blue_nova: Oiê! Tudo Blue por aí? \n \nAgora a liderança receberá de forma automática o relatório de banco de horas do seu time via Blue. Demais, né?:star-struck: Então você já sabe, precisou de ajuda? Chama a Blue!!! \n \nVocê pode validar, por favor? É importante checar a possibilidade de baixa do banco de horas ou apontamentos não realizados. Feshow? \n \nBeijinhos da Blue! :blue_heart:',file=arq)
                        time.sleep(1)
                    except:
            ...
            #People analyts        
                ...
                for i in event['files']:
                    if i['url_private_download'] and i['mode']=='snippet':    
                        ...
                        r = requests.get(url, auth=BearerAuth(token))
                        file_data = r.content   # get binary content
                        file_name="hierarquia.csv"
                        # save file to disk
                        with open(file_name , 'w+b') as f:
                            f.write(bytearray(file_data))
                result=client.chat_postMessage(channel=channel_id,text='Separando os atendimentos por gestor..',thread_ts=thread)    
                caminho=cwd + '/hierarquia.csv'
                df=pd.read_csv(caminho,sep=';')                   
                for i in df.index:
                    e_mail=str(df['Destinatário'][i]).lower()
                    try:
                        id_usu=client.users_lookupByEmail(token=os.environ.get("SLACK_BOT_TOKEN"),email=e_mail)['user']['id']
                        time.sleep(2)
                        canal_gestor=dic.get(id_usu)
                        if canal_gestor!='None':
                            peopleHerarquia=rdHierarchyPeopleAnalytics.post(str(df['Nome Funcionário/Vaga'][i]),str(df['Macro Área'][i]),str(df['Área'][i]),str(df['Email'][i]),str(df['Nome Gestor'][i]),str(df['CC'][i]),str(df['BU'][i]),str(df['Tribo'][i]),str(df['Squad'][i]),str(df['BU_2'][i]),str(df['Tribo_2'][i]),str(df['Squad_2'][i]),str(df['Nome_N0'][i]),str(df['Nome_N1'][i]),str(df['Nome_N2'][i]),str(df['Nome_N3'][i]),str(df['Nome_N4'][i]),str(df['Nome_N5'][i]),str(df['Nome_N6'][i]),str(df['Nome_N7'][i]),str(df['AREA_1'][i]),str(df['AREA_2'][i]),str(df['AREA_3'][i]),str(df['AREA_4'][i]),str(df['AREA_5'][i]),str(df['AREA_6'][i]),str(df['AREA_7'][i]),str(df['Destinatário'][i]),str(canal_gestor).replace("[","").replace("]","").replace("'",""))
                    except:
